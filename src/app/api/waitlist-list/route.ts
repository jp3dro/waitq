...
export async function GET(req: Request) {
  ...
  let selectFields = isAdmin
    ? "id, customer_name, phone, email, status, queue_position, created_at, notified_at, cancelled_at, updated_at, ticket_number, token, send_sms, send_whatsapp, party_size, seating_preference, sms_message_id, sms_status, sms_sent_at, sms_delivered_at, sms_error_message, whatsapp_message_id, whatsapp_status, whatsapp_sent_at, whatsapp_delivered_at, whatsapp_error_message"
    : "id, customer_name, status, queue_position, created_at, notified_at, cancelled_at, updated_at, ticket_number, party_size, seating_preference";
  ...
  if (error && (error.message.includes("send_sms") || error.message.includes("send_whatsapp") || error.message.includes("sms_message_id") || error.message.includes("whatsapp_message_id") || error.message.includes("email") || error.message.includes("column"))) {
    selectFields = isAdmin
      ? "id, customer_name, phone, email, status, queue_position, created_at, notified_at, cancelled_at, updated_at, ticket_number, token, party_size, seating_preference"
      : "id, customer_name, status, queue_position, created_at, notified_at, cancelled_at, updated_at, ticket_number, party_size, seating_preference";
    q = supabase
      .from("waitlist_entries")
      .select(selectFields)
