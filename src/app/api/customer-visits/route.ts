...
export async function GET(req: NextRequest) {
  ...
  const baseSelect = "id, customer_name, phone, email, party_size, seating_preference, status, created_at, notified_at, cancelled_at, cancelled_by, waitlist_id, visits_count, is_returning";
  ...
  if (
    ...
    let q2 = admin
      .from("waitlist_entries")
      .select("id, customer_name, phone, email, party_size, seating_preference, status, created_at, notified_at, cancelled_at, cancelled_by, waitlist_id", { count: "exact" })
      .eq("business_id", businessId)
      .gte("created_at", fromDate.toISOString())
      .order("created_at", { ascending: false })
      .range((page - 1) * pageSize, page * pageSize - 1);
