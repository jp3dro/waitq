...
type FormValues = { phone: string; customerName: string; waitlistId: string };

export default function AddForm({ onDone }: { onDone?: () => void }) {
  const [isPending, startTransition] = useTransition();
  const [message, setMessage] = useState<string | null>(null);
  const { register, handleSubmit, reset, watch } = useForm<FormValues>({
    defaultValues: { phone: "", customerName: "", waitlistId: "" },
  });
  ...
  const onSubmit = (values: FormValues) => {
    ...
    startTransition(async () => {
      ...
      if (res.ok) {
        reset({ phone: "", customerName: "", waitlistId: values.waitlistId });
